...
void MD5Update (MD5_CTX *mdContext, const BYTE *inBuf, unsigned int inLen)
{
	UINT4 in[16];
	int mdi = 0;
...
int md5file ( char *fn , unsigned long seed , MD5_CTX *mdContext )
{
	...
	for (;;) {
		...
		trb += rb ;
		MD5Update ( mdContext , (BYTE *) buf , rb ) ;
...
char * GetMD5Hash(char szHashOut[33],const BYTE *pBufIn, unsigned long nInSize)
{
	MD5_CTX m_md5;

	MD5Init(&m_md5);
	MD5Update(&m_md5, pBufIn, nInSize);
	MD5Final(&m_md5);
	szHashOut[0] = '\0';
	for ( int i = 0 ; i < 16 ; i++ )
	{
...
char * GetMD5Hash16(char szHashOut[17], const BYTE *pBufIn, unsigned long nInSize)
{
	MD5_CTX m_md5;
...
BYTE * GetMd5HashHex(char* buf  ,unsigned long  buflen,BYTE* outputmd5)
{	
	MD5_CTX context;

	MD5Init (&context);
	MD5Update (&context, (BYTE*)buf, buflen);
	MD5Final (&context);
	memcpy(outputmd5,context.digest,16*sizeof(char));
	return outputmd5;
